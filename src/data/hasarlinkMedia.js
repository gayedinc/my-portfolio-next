export const hasarlinkMedia = {
  hero: {
    variant: 'heroDevices',
    priority: true,

    images: [
      {
        key: 'hero-web',
        src: '/images/hasarlink/hero/web-masaustu.png',
        type: 'web',
        width: 1580,
        height: 1013,
        altKey: 'hasarlink_media_web_desktop_alt',
        placeholderKey: 'hasarlink_media_web_placeholder',
      },

      {
        key: 'hero-mobile-new',
        src: '/images/hasarlink/mobile/hsrlnk-mobil-anasayfa.png',
        type: 'phone',
        width: 531,
        height: 1050,
        altKey: 'hasarlink_media_mobile_home_alt',
        placeholderKey: 'hasarlink_media_hero_placeholder',
      },
    ],
  },

  decisions: {
    commonFlow: {
      variant: 'phoneSlider',
      captionKey: 'hasarlink_media_common_flow_caption',

      images: [
        {
          key: 'insurance-select-new',
          src: '/images/hasarlink/mobile/hsrlnk-sigorta-secme-ekrani.png',
          type: 'phone',
          width: 531,
          height: 1050,
          altKey: 'hasarlink_media_insurance_select_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },

        {
          key: 'opening-type-new',
          src: '/images/hasarlink/mobile/hsrlnk-sigorta-nereden-aciliyor.png',
          type: 'phone',
          width: 531,
          height: 1050,
          altKey: 'hasarlink_media_opening_type_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },
      ],
    },

    guidedForm: {
      variant: 'phoneSlider',
      captionKey: 'hasarlink_media_guided_form_caption',

      images: [
        {
          key: 'victim-form-new',
          src: '/images/hasarlink/mobile/hsrlnk-magdur-bilgileri.png',
          type: 'phone',
          width: 531,
          height: 1050,
          altKey: 'hasarlink_media_victim_form_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },

        {
          key: 'document-upload-new',
          src: '/images/hasarlink/mobile/hsrlnk-evrak-yukleme-alani.png',
          type: 'phone',
          width: 531,
          height: 1050,
          altKey: 'hasarlink_media_document_upload_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },

        {
          key: 'form-summary-new',
          src: '/images/hasarlink/mobile/hsrlnk-form-adim-1.png',
          type: 'phone',
          width: 531,
          height: 1050,
          altKey: 'hasarlink_media_form_summary_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },
      ],
    },

    fileOverview: {
      variant: 'phoneSingle',

      images: [
        {
          key: 'file-list-new',
          src: '/images/hasarlink/mobile/hsrlnk-dosya-bildirimlerim.png',
          type: 'phone',
          width: 531,
          height: 1050,
          altKey: 'hasarlink_media_file_list_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },
      ],
    },

    fileDetail: {
      variant: 'longPhone',

      images: [
        {
          key: 'file-detail-new',
          src: '/images/hasarlink/mobile/hsrlnk-dosya-bildirimlerim-detay.png',
          type: 'phone',
          width: 531,
          height: 1050,
          variant: 'longPhone',
          altKey: 'hasarlink_media_file_detail_alt',
          placeholderKey: 'hasarlink_media_mobile_placeholder',
        },
      ],
    },
  },
};